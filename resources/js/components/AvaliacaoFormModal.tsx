// resources/js/components/AvaliacaoFormModal.tsx (CORRIGIDO)
...
export default function AvaliacaoFormModal({ isOpen, onClose, turmaId, materias }: AvaliacaoFormModalProps) {
    // REMOVIDO o campo 'nota_inicial' do estado do formulário
                        ...
                        <TransitionChild as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <DialogPanel className="w-full max-w-md transform overflow-hidden rounded-2xl border bg-card p-6 text-left align-middle shadow-xl transition-all">
                                ...
                                <p className="mt-1 text-sm text-muted-foreground">
                                    Esta ação criará um registro de avaliação com nota 0 para todos os alunos desta turma.
                                </p>

                                <form onSubmit={submit} className="mt-4 space-y-6">
                                    {/* Campo Matéria */}
                                    <div>
                                        <label htmlFor="materia_id" className="block text-sm font-medium text-muted-foreground">Matéria</label>
                                        <select
                                            id="materia_id"
                                    ...
                                    <div>
                                        <label htmlFor="data_avaliacao" className="block text-sm font-medium text-muted-foreground">Data da Avaliação</label>
                                        <input
                                            id="data_avaliacao"
                                            type="date"
                                    ...
                                    {/* REMOVIDA A DIV INTEIRA DO CAMPO "Nota Inicial" */}
